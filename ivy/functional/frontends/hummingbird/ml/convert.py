...
def _convert_common(model, backend, test_input=None, device="cpu", extra_config={}):
    ...
    # By default we return the converted model wrapped into a
    # `hummingbird.ml._container.SklearnContainer` object.
    if constants.CONTAINER not in extra_config:
        extra_config[constants.CONTAINER] = True
    # By default we set num of intra-op parallelism to be
...
def _convert_sklearn(model, backend, test_input, device, extra_config={}):
    """
    This function converts the specified *scikit-learn* (API) model into its *backend*
    counterpart.
    ...
    hb_model = topology_converter(
        topology, backend, test_input, device, extra_config=extra_config
    )
    return hb_model
...
def convert(model, backend="pytorch", test_input=None, device="cpu", extra_config={}):
